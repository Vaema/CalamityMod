...
{ 
    internal static class AstralSnowBG
    {
        public static void Load()
        {
            ...
            On_Main.DrawSurfaceBG += DrawAstralSnowBG;
        }
        ...
        private static void DrawAstralSnowBG(On_Main.orig_DrawSurfaceBG orig, Main self)
        {
            if (Main.gameMenu)
            {
                orig(self);
            ...
            if (Main.LocalPlayer.InModBiome(ModContent.GetInstance<AbovegroundAstralSnowBiome>()))
            {
            ...
            if (Transparency > 0f)
            {
                ...
                for (int Layers = 4; Layers >= 0; Layers--)
                {
                    ...
                    Texture2D BGTexture = ModContent.Request<Texture2D>("CalamityMod/Backgrounds/AstralSnowBG" + Layers).Value;
                    ...
                    for (int j = num6 - 2; j < num6 + 4 + (int)(Main.screenWidth / RectangleScale); j++)
                    {
                        ...
                        if (Layers < 2)
                        {
                            Texture2D BGTextureGlow = ModContent.Request<Texture2D>("CalamityMod/Backgrounds/AstralSnowBG" + Layers + "_Glow").Value;
