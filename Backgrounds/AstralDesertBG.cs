...
{ 
    internal static class AstralDesertBG
    {
        ...
        private static void DrawAstralSnowBG(On_Main.orig_DrawSurfaceBG orig, Main self)
        {
            ...
            if (Main.LocalPlayer.InModBiome(ModContent.GetInstance<AbovegroundAstralDesertBiome>()))
            {
            ...
            if (Transparency > 0f)
            {
                ...
                for (int Layers = 2; Layers >= 0; Layers--)
                {
                    //get each background texture
                    Texture2D BGTexture = ModContent.Request<Texture2D>("CalamityMod/Backgrounds/AstralDesertBG" + Layers).Value;
