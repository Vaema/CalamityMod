...
namespace CalamityMod.Tiles
{
    public class CalamityGlobalTile : GlobalTile
    {
        ...
        {
            if (type == TileID.Cannon)
            {
                int cannonType = Main.tile[i, j].TileFrameX / 72;
                
                Player p = Main.LocalPlayer;
                p.HeldItem.makeNPC = -1;

                if (cannonType == 0 && p.ItemTimeIsZero && p.controlUseTile && p.HeldItem.type == ModContent.ItemType<BabyCannonballJellyfishItem>() 
                    && p.position.X / 16f - (float)Player.tileRangeX - (float)p.HeldItem.tileBoost - (float)p.blockRange <= (float)Player.tileTargetX 
                    && (p.position.X + (float)p.width) / 16f + (float)Player.tileRangeX + (float)p.HeldItem.tileBoost - 1f + (float)p.blockRange >= (float)Player.tileTargetX 
                    && p.position.Y / 16f - (float)Player.tileRangeY - (float)p.HeldItem.tileBoost - (float)p.blockRange <= (float)Player.tileTargetY 
                    && (p.position.Y + (float)p.height) / 16f + (float)Player.tileRangeY + (float)p.HeldItem.tileBoost - 2f + (float)p.blockRange >= (float)Player.tileTargetY)
                {

                    int tileX = Main.tile[i, j].TileFrameX / 18;
                    int angle = 0;
                    while (tileX >= 4)
                    {
                        tileX -= 4;
                    ...
                    tileX = i - tileX;
                    int tileY;
                    for (tileY = Main.tile[i, j].TileFrameY / 18; tileY >= 3; tileY -= 3)
                    {
                        angle++;
                    }
                    tileY = j - tileY;
                    p.ApplyItemTime(p.HeldItem);
                    p.ConsumeItem(ModContent.ItemType<BabyCannonballJellyfishItem>());
                    ...
                    int jellyfishb = Projectile.NewProjectile(new EntitySource_TileInteraction(p, tileX, tileY), spawnPosition.X, spawnPosition.Y, speedX, speedY, ModContent.ProjectileType<SulphuricBlast>(), p.HeldItem.damage, 8f, p.whoAmI);
