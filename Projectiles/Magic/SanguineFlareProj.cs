...
namespace CalamityMod.Projectiles.Magic
{
    public class SanguineFlareProj : ModProjectile, ILocalizedModType
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center, Projectile.velocity.RotatedByRandom(1) * Main.rand.NextFloat(0.75f, 1.25f), ModContent.ProjectileType<BloodstoneHealOrb>(), 1, 0f, player.whoAmI);
            }
            if (dmgMult > 1)
            {
