...
{
    public class FrostbatMinion : BaseMinionProjectile
    {
        public override int AssociatedProjectileTypeID => ProjectileType<FrostbatMinion>();

        public override int AssociatedBuffTypeID => BuffType<FrostbatBuff>();

        public override ref bool AssociatedMinionBool => ref ModdedOwner.FrostbatBool;
        ...
        public override void MinionAI()
        {
            ...
            if (!Main.dedServ)
            {
                ...
                {
                    Dust ambientDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.IceTorch);
        ...
        private void AttackBehavior()
        {
            ...
            if (!AttackState.HasFlag(AttackBehaviorFlags.HasDashed) && Vector2.Dot(Projectile.DirectionTo(Target.Center), Projectile.velocity.SafeNormalize(-Vector2.UnitY)) < 0.96f)
            {
                Projectile.velocity = Projectile.velocity.ToRotation().AngleTowards(Projectile.DirectionTo(Target.Center).ToRotation(), 0.08f).ToRotationVector2() *
                    Utils.Remap(Projectile.DistanceSQ(Target.Center), 6400f, 0f, 8f, 0f);
            }
            ...
            {
                Projectile.velocity = CalamityUtils.CalculatePredictiveAimToTarget(Projectile.Center, Target, 18f);
            ...
            if (AttackState.HasFlag(AttackBehaviorFlags.IsReturningToOwner))
            {
                Projectile.velocity = Projectile.velocity.ToRotation().AngleTowards(Projectile.DirectionTo(IdlePosition).ToRotation(), 0.08f).ToRotationVector2() *
                    Utils.Remap(Projectile.DistanceSQ(IdlePosition), 6400f, 0f, 8f, 0f);
