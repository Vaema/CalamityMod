    ...
    public class DiamondOfTheDeepProjectile : ModProjectile, ILocalizedModType
    {
                ...
                else // Attacking
                {
                    if (time > startTime)
                    {
                        ...
                        if (projType == 2)
                        {
                            if (hitCooldown > 0)
                                hitCooldown--;
                            float timeLerp = Utils.GetLerpValue(400, 800, Projectile.timeLeft, true);
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            if (projType == 0)
            {
                ...
                Projectile blast = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<BasicBurstExclusive>(), Projectile.damage, -5, Projectile.owner, blastSize, minMultiplier, hitsToMinMult);
                blast.timeLeft = 3;
