...
namespace CalamityMod.Projectiles.Ranged
{
    public class MortarRoundProj : ModProjectile, ILocalizedModType
    {
        ...
        internal static void SpawnDust(Projectile p)
        {
            ...
            for (int d = 0; d < 70; d++)
            {
                int fire = Dust.NewDust(p.position, p.width, p.height, DustID.Torch, 0f, 0f, 100, default, 3f);
                Main.dust[fire].noGravity = true;
                Main.dust[fire].velocity *= 5f;
                fire = Dust.NewDust(p.position, p.width, p.height, DustID.Torch, 0f, 0f, 100, default, 2f);
