...
namespace CalamityMod.Projectiles.Ranged
{
    public class BloodfireBulletProj : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            // Lighting
            Lighting.AddLight(Projectile.Center, (!ChildSafety.Disabled ? Main.DiscoColor : Color.Red).ToVector3() * 0.5f);
            ...
            if (Projectile.localAI[0] > 6f)
            {
                ...
                if (targetDist < 1400f)
                {
                    SparkParticle spark = new SparkParticle(Projectile.Center - Projectile.velocity, -Projectile.velocity * 0.01f, false, 4, 0.4f, !ChildSafety.Disabled ? Main.DiscoColor : Color.Firebrick);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (Projectile.localAI[0] > 6f)
            {
                CalamityUtils.DrawAfterimagesFromEdge(Projectile, 0, (!ChildSafety.Disabled ? Main.DiscoColor : Color.Red));
            }
            return true;
        }
        ...
        private float OnHitEffect(Player owner, NPC target)
        {
            ...
            if (lifeRegenTimeContribution == 0.1f) // Special hit visual if the bonus damage is at the cap.
            {
                for (int k = 0; k < 3; k++)
                {
                    BloodParticle blood = new BloodParticle(Projectile.Center, new Vector2(6.5f, 6.5f).RotatedByRandom(100) * Main.rand.NextFloat(0.8f, 1.2f), Main.rand.Next(8, 10 + 1), Main.rand.NextFloat(0.7f, 0.9f), !ChildSafety.Disabled ? Main.DiscoColor : Color.Red);
        ...
        public override void OnKill(int timeLeft)
        {
            for (int k = 0; k < 3; k++)
            {
                SparkParticle spark = new SparkParticle(Projectile.Center, -Projectile.velocity.RotatedByRandom(0.5) * Main.rand.NextFloat(1f, 3f), false, Main.rand.Next(5, 7 + 1), Main.rand.NextFloat(0.4f, 0.6f), !ChildSafety.Disabled ? Main.DiscoColor : Color.Red);
                GeneralParticleHandler.SpawnParticle(spark);
            }
            for (int k = 0; k < 8; k++)
            {
                Dust dust = Dust.NewDustPerfect(Projectile.Center, (!ChildSafety.Disabled ? DustID.RainbowMk2 : (Main.rand.NextBool(3) ? 130 : 60)), new Vector2(3, 3).RotatedByRandom(100) * Main.rand.NextFloat(0.5f, 1.5f));
                dust.noGravity = true;
                dust.scale = Main.rand.NextFloat(0.5f, 0.9f);
                if (dust.type == 130)
                    dust.scale = Main.rand.NextFloat(0.35f, 0.55f);
