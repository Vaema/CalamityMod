            ...
            else
            {
                ...
                if (CanHit)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        GeneralParticleHandler.SpawnParticle(new HeavySmokeParticle(particlePos, -particleVel.RotatedByRandom(0.2f) * 2, Main.rand.NextBool(4) ? Color.AliceBlue : Color.DodgerBlue, 23, Main.rand.NextFloat(0.2f, 0.6f), 0.65f, 0, true));
                    }
