...
public class OmegaBlue : CooldownHandler
{
    ...
    public override void OnCompleted()
    {
        for (int i = 0; i < 66; i++)
        {
            Dust dust = Dust.NewDustDirect(instance.player.position, instance.player.width, instance.player.height, DustID.PurificationPowder, 0, 0, 100, Color.Transparent, 2.6f);
            dust.noGravity = true;
            dust.noLight = true;
            dust.fadeIn = 1f;
            dust.velocity *= 6.6f;
